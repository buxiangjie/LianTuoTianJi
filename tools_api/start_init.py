...
if sys.platform in ("darwin", "linux"):
	...
	if port == "":
		...
		Ulog.info(f"获取到的端口为:{data}")
		pid_list = re.findall('uvicorn(.*?)root', data)
		Ulog.info(f"匹配到的PID为:{pid_list}")
		count = 1
		for i in pid_list:
			Ulog.info(f"开始清理第{count}个端口-------------------------------------")
			os.popen(f"kill {str(i)}")
			Ulog.info(f"检查端口{i}是否被kill")
			pid = re.findall('uvicorn(.*?)root', str(os.popen("lsof -i:8817").read()).replace(" ", ""))
			Ulog.info(f"当前端口PID列表为:{pid}")
			if str(i) not in pid:
				Ulog.info("删除成功")
				count += 1
			else:
