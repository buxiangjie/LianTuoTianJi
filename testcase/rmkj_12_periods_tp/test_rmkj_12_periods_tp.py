...
class Rmkj12Tp(unittest.TestCase):
	...
	def test_100_apply(self):
		...
		self.r.set('rmkj_12_periods_projectId', projectId)
	...
	def test_103_sign_borrow(self):
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("rmkj_12_periods_contractId", json.loads(rep.text)['content']['contractId'])
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))
	...
	def test_106_sign(self):
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("rmkj_12_periods_signTaskId", json.loads(rep.text)['data']['signTaskId'])
	...
	def test_112_query_repayment_plan(self):
		...
		logger.info("返回信息:%s" % rep.text)
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))
		self.r.set("rmkj_12_periods_repayment_plan", json.dumps(json.loads(rep.text)['content']['repaymentPlanList']))
	...
	def test_116_deduction_apply(self):
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("rmkj_12_periods_deductionTaskId", json.loads(rep.text)['content']['deductionTaskId'])
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))
