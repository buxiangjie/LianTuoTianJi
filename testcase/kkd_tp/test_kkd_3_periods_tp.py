...
logger = Logger(logger="test_kkd_3_periods_tp").getlog()
...
class Kkd3Tp(unittest.TestCase):
	...
	def test_100_apply(self):
		...
		Common.p2p_get_userinfo('kkd_3_periods', self.env)
		self.r.mset(
			{
				"kkd_3_periods_sourceUserId": Common.get_random('userid'),
				"kkd_3_periods_transactionId": Common.get_random('transactionId'),
				"kkd_3_periods_phone": Common.get_random('phone'),
				"kkd_3_periods_sourceProjectId": Common.get_random('sourceProjectId'),
			}
		)
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get('kkd_3_periods_sourceUserId'),
				"transactionId": self.r.get('kkd_3_periods_transactionId')
			}
		)
		param['personalInfo'].update(
			{
				"cardNum": self.r.get('kkd_3_periods_cardNum'),
				"custName": self.r.get('kkd_3_periods_custName'),
				"phone": self.r.get('kkd_3_periods_phone')
			}
		)
		...
		param.update(
			{
				...
				"transactionId": self.r.get('kkd_3_periods_transactionId'),
				"associationId": self.r.get('kkd_3_periods_projectId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_102_query_apply_result(self):
		...
		param.update(
			{
				"sourceProjectId": self.r.get('kkd_3_periods_sourceProjectId'),
				"projectId": self.r.get('kkd_3_periods_projectId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
		...
		logger.info("返回信息:%s" % response_data)
		self.assertEqual(response_data['resultCode'], int(data[0]['resultCode']))
		self.assertEqual(response_data['content']['auditStatus'], 2)
	...
	def test_103_sign_borrow(self):
		...
		print("返回json:%s" % response_data)
		logger.info("返回信息:%s" % response_data)
		self.r.set("kkd_3_periods_contractId", response_data['content']['contractId'])
		self.assertEqual(response_data['resultCode'], int(data[0]['resultCode']))
	...
	def test_105_image_upload(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		param.update({"associationId": self.r.get('kkd_3_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get("kkd_3_periods_sourceUserId"),
				"contractId": self.r.get("kkd_3_periods_contractId")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_108_loan_pfa(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		self.r.set("kkd_3_periods_loan_serviceSn", Common.get_random("serviceSn"))
		param.update(
			{
				"sourceProjectId": self.r.get("kkd_3_periods_sourceProjectId"),
				"projectId": self.r.get("kkd_3_periods_projectId"),
				"sourceUserId": self.r.get("kkd_3_periods_sourceUserId"),
				"serviceSn": self.r.get("kkd_3_periods_loan_serviceSn"),
				"id": self.r.get('kkd_3_periods_cardNum')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_109_loan_query(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		param.update({"serviceSn": self.r.get("kkd_3_periods_loan_serviceSn")})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		logger.info("返回信息:%s" % response_data)
		self.assertEqual(response_data['resultCode'], int(data[0]['resultCode']))
		self.assertEqual(response_data['content']['projectLoanStatus'], 3)
	...
	def test_111_calculate(self):
		...
		self.r.set(
			...
			json.dumps(response_data['content']['repaymentPlanList'])
		)
		self.assertEqual(response_data['resultCode'], int(data[0]['resultCode']))
	...
	@unittest.skip("跳过")
	def test_115_push_reconciliation_result(self):
		"""对账结果推送"""
		...
		param.update(
			{
				"accountId": Common.get_random("serviceSn"),
			}
		)
	...
	@unittest.skip("-")
	def test_116_capital_flow(self):
		"""资金流水推送"""
