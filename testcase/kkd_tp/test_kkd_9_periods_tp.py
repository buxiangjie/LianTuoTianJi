...
class Kkd9Tp(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		cls.file = Config().get_item('File', 'kkd_case_file')
	...
	def test_100_apply(self):
		...
		Common.p2p_get_userinfo('kkd_9_periods', self.env)
		self.r.mset(
			{
				"kkd_9_periods_sourceUserId": Common.get_random('userid'),
				"kkd_9_periods_transactionId": Common.get_random('transactionId'),
				"kkd_9_periods_phone": Common.get_random('phone'),
				"kkd_9_periods_sourceProjectId": Common.get_random('sourceProjectId'),
			}
		)
		param = json.loads(data[0]['param'])
		param.update(
			{
				"sourceProjectId": self.r.get('kkd_9_periods_sourceProjectId'),
		...
		param['personalInfo'].update(
			{
				"cardNum": self.r.get('kkd_9_periods_cardNum'),
				"custName": self.r.get('kkd_9_periods_custName'),
				"phone": self.r.get('kkd_9_periods_phone')
			}
		)
	...
	def test_102_query_apply_result(self):
		"""进件结果查询"""
		Assert.check_column("wxjk_project", self.env, self.r.get('kkd_9_periods_projectId'))
		...
		param.update(
			{
				"sourceProjectId": self.r.get('kkd_9_periods_sourceProjectId'),
				"projectId": self.r.get('kkd_9_periods_projectId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_103_sign_borrow(self):
		...
		self.r.set("kkd_9_periods_contractId", rep['content']['contractId'])
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
	...
	def test_105_image_upload(self):
		...
		param = json.loads(data[0]['param'])
		param.update({"associationId": self.r.get('kkd_9_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get("kkd_9_periods_sourceUserId"),
				"contractId": self.r.get("kkd_9_periods_contractId")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_107_calculate(self):
		...
		param.update(
			{
				"sourceUserId": self.r.get("kkd_9_periods_sourceUserId"),
				"transactionId": self.r.get("kkd_9_periods_sourceProjectId"),
				"sourceProjectId": self.r.get("kkd_9_periods_sourceProjectId"),
				"projectId": self.r.get("kkd_9_periods_projectId")
			}
		)
	...
	def test_108_loan_pfa(self):
		...
		param = json.loads(data[0]['param'])
		self.r.set("kkd_9_periods_loan_serviceSn", Common.get_random("serviceSn"))
		param.update(
			{
				"sourceProjectId": self.r.get("kkd_9_periods_sourceProjectId"),
				"projectId": self.r.get("kkd_9_periods_projectId"),
				"sourceUserId": self.r.get("kkd_9_periods_sourceUserId"),
				"serviceSn": self.r.get("kkd_9_periods_loan_serviceSn"),
				"id": self.r.get('kkd_9_periods_cardNum')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_109_loan_query(self):
		"""放款结果查询"""
		GetSqlData.loan_set(environment=self.env, project_id=self.r.get('kkd_9_periods_projectId'))
		data = excel_table_byname(self.file, 'pfa_query')
		param = json.loads(data[0]['param'])
		param.update({"serviceSn": self.r.get("kkd_9_periods_loan_serviceSn")})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_110_query_repayment_plan(self):
		...
		Assert.check_repayment(False, self.env, self.r.get("kkd_9_periods_projectId"))
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		self.r.set("kkd_9_periods_repayment_plan", json.dumps(rep['content']['repaymentPlanList']))
	...
	def test_111_calculate(self):
		...
		self.r.set(
			"kkd_9_periods_early_settlement_repayment_plan",
	...
	def test_112_offline_repay_repayment(self):
		...
		repayment_plan_list = self.r.get("kkd_9_periods_repayment_plan")
		success_amount = 0.00
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(True, self.env, self.r.get("kkd_9_periods_projectId"), param)

	@unittest.skip("跳过")
	def test_113_offline_nrepay_early_settlement(self):
		"""线下还款流水推送：提前全部结清"""
		data = excel_table_byname(self.file, 'offline_repay')
		param = json.loads(data[0]['param'])
		plan_pay_date = GetSqlData.get_repayment_plan_date(
		...
		repayment_plan_list = json.loads(self.r.get("kkd_9_periods_early_settlement_repayment_plan"))
		success_amount = 0.00
		repayment_detail_list = []
