...
class JkCkshd6PeriodsTp(unittest.TestCase):
	...
	def test_100_credit_apply(self):
		...
		param['personalInfo'].update(
			{
				"cardNum": self.r.get('jk_ckshd_12_periods_cardNum'),
				"custName": self.r.get('jk_ckshd_12_periods_custName'),
				"phone": self.r.get('jk_ckshd_12_periods_phone')
			}
		)
	...
	def test_101_query_result(self):
		"""授信结果查询"""
		Assert.check_column("jk_ckshd_credit", self.env, self.r.get("jk_ckshd_12_periods_creditId"))
		...
		param = json.loads(data[0]['param'])
		param.update({"creditId": self.r.get('jk_ckshd_12_periods_creditId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_102_query_user_amount(self):
		...
		param.update(
			{
				"sourceUserId": self.r.get('jk_ckshd_12_periods_sourceUserId'),
				"userId": self.r.get('jk_ckshd_12_periods_userId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
		...
		param.update(
			{
				...
				"transactionId": self.r.get('jk_ckshd_12_periods_transactionId'),
				"associationId": self.r.get('jk_ckshd_12_periods_creditId'),
	...
	def test_105_cancel(self):
		...
		param.update(
			{
				"sourceProjectId": self.r.get('jk_ckshd_12_periods_sourceProjectId'),
				"projectId": self.r.get('jk_ckshd_12_periods_projectId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_106_query_apply_result(self):
		"""进件结果查询"""
		Assert.check_column("jk_ckshd_project", self.env, self.r.get("jk_ckshd_12_periods_projectId"))
	...
	def test_108_image_upload(self):
		...
		param = json.loads(data[0]['param'])
		param.update({"associationId": self.r.get('jk_ckshd_12_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get("jk_ckshd_12_periods_sourceUserId"),
				"contractId": self.r.get("jk_ckshd_12_periods_contractId")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_110_calculate(self):
		...
		param.update(
			{
				"sourceUserId": self.r.get("jk_ckshd_12_periods_sourceUserId"),
				"transactionId": self.r.get("jk_ckshd_12_periods_sourceProjectId"),
				"sourceProjectId": self.r.get("jk_ckshd_12_periods_sourceProjectId"),
				"projectId": self.r.get("jk_ckshd_12_periods_projectId")
			}
		)
	...
	def test_111_deduction_share_sign(self):
		...
		param.update(
			{
				...
				"sourceProjectId": self.r.get("jk_ckshd_12_periods_sourceProjectId"),
				"projectId": self.r.get("jk_ckshd_12_periods_projectId"),
				"name": self.r.get("jk_ckshd_12_periods_custName"),
				"cardNo": self.r.get("jk_ckshd_12_periods_cardNum"),
				# "bankNo": "6217002200003225702",
				"bankNo": self.r.get("jk_ckshd_12_periods_bankcard"),
				"bankPhone": self.r.get("jk_ckshd_12_periods_phone"),
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		self.r.set("jk_ckshd_12_periods_signId", rep["content"]["signId"])
	...
	def test_112_deduction_share_sign(self):
		...
		param.update(
			{
				"associationId": self.r.get("jk_ckshd_12_periods_signId"),
	...
	def test_113_loan_pfa(self):
		...
		param = json.loads(data[0]['param'])
		self.r.set("jk_ckshd_12_periods_loan_serviceSn", Common.get_random("serviceSn"))
		param.update(
			{
				"sourceProjectId": self.r.get("jk_ckshd_12_periods_sourceProjectId"),
				"projectId": self.r.get("jk_ckshd_12_periods_projectId"),
	...
	def test_114_loan_query(self):
		"""放款结果查询"""
		GetSqlData.loan_set(environment=self.env, project_id=self.r.get('jk_ckshd_12_periods_projectId'))
		data = excel_table_byname(self.file, 'pfa_query')
		param = json.loads(data[0]['param'])
		param.update({"serviceSn": self.r.get("jk_ckshd_12_periods_loan_serviceSn")})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_115_query_repayment_plan(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(False, self.env, self.r.get("jk_ckshd_12_periods_projectId"))
		self.r.set("jk_ckshd_12_periods_repayment_plan", json.dumps(rep['content']['repaymentPlanList']))
	...
	def test_116_calculate(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		self.r.set("jk_ckshd_12_periods_early_settlement_repayment_plan",
	...
	def test_118_offline_repay_repayment(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(True, self.env, self.r.get("jk_ckshd_12_periods_projectId"), param)
