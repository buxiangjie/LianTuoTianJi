...
class JqhRepayment(unittest.TestCase):
	...
	def test_0_approved(self):
		...
		Common.p2p_get_userinfo("jqh_repayment", self.env)
		self.r.mset(
			{
		...
		param.update(
			{
				"sourceProjectId": self.r.get("jqh_repayment_sourceProjectId"),
				"sourceUserId": self.r.get("jqh_repayment_sourceUserId"),
				"transactionId": self.r.get("jqh_repayment_transactionId")
			}
		)
		param['applyInfo'].update({"applyTime": Common.get_time("-")})
		param['personalInfo'].update(
			{
				"cardNum": self.r.get("jqh_repayment_cardNum"),
				"custName": self.r.get("jqh_repayment_custName"),
				"phone": self.r.get("jqh_repayment_phone")
			}
		)
		param['cardInfo'].update({"bankPhone": self.r.get("jqh_repayment_phone")})
		...
		self.r.set("jqh_repayment_projectId", json.loads(rep.text)['content']['projectId'])
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['msgCode']))

	def test_1_loan(self):
		"""借去花放款接口"""
		GetSqlData.change_project_audit_status(
			project_id=self.r.get("jqh_repayment_projectId"),
			enviroment=self.env
		)
		...
		param.update(
			{
				"sourceProjectId": self.r.get("jqh_repayment_sourceProjectId"),
				"sourceUserId": self.r.get("jqh_repayment_sourceUserId"),
				"projectId": self.r.get("jqh_repayment_projectId"),
				"serviceSn": "SaasL-" + Common.get_random("serviceSn"),
				"id": self.r.get("jqh_repayment_cardNum"),
				"bankPhone": self.r.get("jqh_repayment_phone")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
