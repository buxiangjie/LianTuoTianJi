...
logger = Logger(logger="test_ddq_9_periods_tp").getlog()
...
class Ddq9Tp(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		file = Config().get_item('File', 'ddq_9_periods_case_file')
	...
	def test_100_apply(self):
		...
		self.r.set('ddq_9_periods_projectId', projectId)
	...
	def test_103_sign_borrow(self):
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("ddq_9_periods_contractId", json.loads(rep.text)['content']['contractId'])
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))
	...
	def test_109_loan_query(self):
		"""放款结果查询"""
		GetSqlData.loan_set(enviroment=self.env, project_id=self.r.get('ddq_9_periods_projectId'))
	...
	def test_110_query_repayment_plan(self):
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("ddq_9_periods_repayment_plan", json.dumps(json.loads(rep.text)['content']['repaymentPlanList']))
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))
