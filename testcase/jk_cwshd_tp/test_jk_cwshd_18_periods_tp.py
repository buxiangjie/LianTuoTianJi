...
class JkCwshd18PeriodsTp(unittest.TestCase):
	...
	def test_100_credit_apply(self):
		...
		Common.p2p_get_userinfo('jk_cwshd_18_periods', self.env)
		self.r.mset(
			{
				"jk_cwshd_18_periods_sourceUserId": Common.get_random('userid'),
				'jk_cwshd_18_periods_transactionId': Common.get_random('transactionId'),
				"jk_cwshd_18_periods_phone": Common.get_random('phone'),
				"jk_cwshd_18_periods_firstCreditDate": Common.get_time()
			}
		)
		...
		self.r.mset(
			{
				"jk_cwshd_18_periods_creditId": rep['content']['creditId'],
				"jk_cwshd_18_periods_userId": rep['content']['userId']
			}
		)
	...
	def test_101_query_result(self):
		...
		GetSqlData.credit_set(
			environment=self.env,
			credit_id=self.r.get("jk_cwshd_18_periods_creditId")
		)
		data = excel_table_byname(self.file, 'query_result')
		param = json.loads(data[0]['param'])
		param.update({"creditId": self.r.get('jk_cwshd_18_periods_creditId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_102_query_user_amount(self):
		...
		param.update(
			{
				"sourceUserId": self.r.get('jk_cwshd_18_periods_sourceUserId'),
				"userId": self.r.get('jk_cwshd_18_periods_userId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
		...
		param.update(
			{
				...
				"transactionId": self.r.get('jk_cwshd_18_periods_transactionId'),
				"associationId": self.r.get('jk_cwshd_18_periods_creditId'),
	...
	def test_106_query_apply_result(self):
		"""进件结果查询"""
		Assert.check_column("jk_cwshd_project", self.env, self.r.get('jk_cwshd_18_periods_projectId'))
		...
		param.update(
			{
				...
				"contractType": 2,
				"transactionId": self.r.get('jk_cwshd_18_periods_transactionId'),
				"associationId": self.r.get('jk_cwshd_18_periods_projectId'),
	...
	def test_108_image_upload(self):
		...
		param = json.loads(data[0]['param'])
		param.update({"associationId": self.r.get('jk_cwshd_18_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get("jk_cwshd_18_periods_sourceUserId"),
				"contractId": self.r.get("jk_cwshd_18_periods_contractId")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_111_deduction_share_sign(self):
		...
		param.update(
			{
				...
				"sourceProjectId": self.r.get("jk_cwshd_18_periods_sourceProjectId"),
				"projectId": self.r.get("jk_cwshd_18_periods_projectId"),
				"name": self.r.get("jk_cwshd_18_periods_custName"),
				"cardNo": self.r.get("jk_cwshd_18_periods_cardNum"),
				# "bankNo": "6217002200003225702",
				"bankNo": self.r.get("jk_cwshd_18_periods_bankcard"),
				"bankPhone": self.r.get("jk_cwshd_18_periods_phone"),
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		self.r.set("jk_cwshd_18_periods_signId", rep["content"]["signId"])
	...
	def test_112_deduction_share_sign(self):
		...
		param.update(
			{
				"associationId": self.r.get("jk_cwshd_18_periods_signId"),
	...
	def test_113_loan_pfa(self):
		...
		param = json.loads(data[0]['param'])
		self.r.set("jk_cwshd_18_periods_loan_serviceSn", Common.get_random("serviceSn"))
		param.update(
			{
				"sourceProjectId": self.r.get("jk_cwshd_18_periods_sourceProjectId"),
				"projectId": self.r.get("jk_cwshd_18_periods_projectId"),
				"sourceUserId": self.r.get("jk_cwshd_18_periods_sourceUserId"),
				"serviceSn": self.r.get("jk_cwshd_18_periods_loan_serviceSn"),
				"id": self.r.get('jk_cwshd_18_periods_cardNum'),
				"accountName": self.r.get("jk_cwshd_18_periods_custName"),
				"amount": 50000
			}
		)
		if len(data[0]['headers']) == 0:
	...
	def test_114_loan_query(self):
		"""放款结果查询"""
		GetSqlData.loan_set(environment=self.env, project_id=self.r.get('jk_cwshd_18_periods_projectId'))
		data = excel_table_byname(self.file, 'pfa_query')
		param = json.loads(data[0]['param'])
		param.update({"serviceSn": self.r.get("jk_cwshd_18_periods_loan_serviceSn")})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_116_calculate(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		self.r.set(
			"jk_cwshd_18_periods_early_settlement_repayment_plan",
	...
	def test_117_calculate(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		self.r.set(
			"jk_cwshd_18_periods_return_repayment_plan",
			json.dumps(rep['content']['repaymentPlanList'])
		)
