...
class JkCkshd6PeriodsTp(unittest.TestCase):
	...
	def test_102_query_user_amount(self):
		...
		param.update(
			{
				"sourceUserId": self.r.get('jk_cwshd_6_periods_sourceUserId'),
				"userId": self.r.get('jk_cwshd_6_periods_userId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
		...
		param.update(
			{
				...
				"transactionId": self.r.get('jk_cwshd_6_periods_transactionId'),
				"associationId": self.r.get('jk_cwshd_6_periods_creditId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_104_apply(self):
		...
		self.r.mset(
			{
				...
				"jk_cwshd_6_periods_transactionId": Common.get_random('transactionId'),
				"jk_cwshd_6_periods_phone": Common.get_random('phone'),
				"jk_cwshd_6_periods_sourceProjectId": Common.get_random('sourceProjectId'),
			}
		)
		param = json.loads(data[0]['param'])
		param.update(
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		self.r.set('jk_cwshd_6_periods_projectId', rep['content']['projectId'])
	...
	def test_106_query_apply_result(self):
		...
		GetSqlData.change_project_audit_status(
			project_id=self.r.get('jk_cwshd_6_periods_projectId'),
			environment=self.env
		)
	...
	def test_107_sign_borrow(self):
		...
		param.update(
			{
				...
				"sourceContractId": Common.get_random('userid'),
				"transactionId": self.r.get('jk_cwshd_6_periods_transactionId'),
				"associationId": self.r.get('jk_cwshd_6_periods_projectId'),
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		self.r.set("jk_cwshd_6_periods_contractId", rep['content']['contractId'])
	...
	def test_108_image_upload(self):
		...
		param = json.loads(data[0]['param'])
		param.update({"associationId": self.r.get('jk_cwshd_6_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get("jk_cwshd_6_periods_sourceUserId"),
				"contractId": self.r.get("jk_cwshd_6_periods_contractId")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_110_calculate(self):
		...
		param.update(
			{
				"sourceUserId": self.r.get("jk_cwshd_6_periods_sourceUserId"),
				"transactionId": self.r.get("jk_cwshd_6_periods_sourceProjectId"),
				"sourceProjectId": self.r.get("jk_cwshd_6_periods_sourceProjectId"),
				"projectId": self.r.get("jk_cwshd_6_periods_projectId")
			}
		)
		...
		param.update(
			{
				...
				"sourceProjectId": self.r.get("jk_cwshd_6_periods_sourceProjectId"),
				"projectId": self.r.get("jk_cwshd_6_periods_projectId"),
				"name": self.r.get("jk_cwshd_6_periods_custName"),
				"cardNo": self.r.get("jk_cwshd_6_periods_cardNum"),
	...
	def test_112_deduction_share_sign(self):
		...
		param.update(
			{
				"associationId": self.r.get("jk_cwshd_6_periods_signId"),
	...
	def test_113_loan_pfa(self):
		...
		param = json.loads(data[0]['param'])
		self.r.set("jk_cwshd_6_periods_loan_serviceSn", Common.get_random("serviceSn"))
		param.update(
			{
				"sourceProjectId": self.r.get("jk_cwshd_6_periods_sourceProjectId"),
				"projectId": self.r.get("jk_cwshd_6_periods_projectId"),
	...
	def test_114_loan_query(self):
		"""放款结果查询"""
		GetSqlData.loan_set(environment=self.env, project_id=self.r.get('jk_cwshd_6_periods_projectId'))
		data = excel_table_byname(self.file, 'pfa_query')
		param = json.loads(data[0]['param'])
		param.update({"serviceSn": self.r.get("jk_cwshd_6_periods_loan_serviceSn")})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_117_calculate(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		self.r.set(
			"jk_cwshd_6_periods_return_repayment_plan",
			json.dumps(rep['content']['repaymentPlanList'])
		)
	...
	def test_118_offline_repay_repayment(self):
		...
		repayment_plan_list = self.r.get("jk_cwshd_6_periods_repayment_plan")
		success_amount = 0.00
	...
	def test_119_offline_repay_early_settlement(self):
		...
		repayment_plan_list = json.loads(self.r.get("jk_cwshd_6_periods_early_settlement_repayment_plan"))
		success_amount = 0.00
		repayment_detail_list = []
	...
	def test_120_offline_return(self):
		...
		repayment_plan_list = json.loads(self.r.get("jk_cwshd_6_periods_return_repayment_plan"))
		success_amount = 0.00
		repayment_detail_list = []
		for i in repayment_plan_list:
			plan_detail = {
