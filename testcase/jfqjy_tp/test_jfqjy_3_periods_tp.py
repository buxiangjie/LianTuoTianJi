...
logger = Logger(logger="test_jfqjy_3_periods_tp").getlog()


class Jfqjy3Tp(unittest.TestCase):
	...
	def test_100_apply(self):
		...
		Common.p2p_get_userinfo('jfqjy_3_periods', self.env)
		self.r.mset(
			{
				"jfqjy_3_periods_sourceUserId": Common.get_random('userid'),
				"jfqjy_3_periods_transactionId": Common.get_random('transactionId'),
				"jfqjy_3_periods_phone": Common.get_random('phone'),
				"jfqjy_3_periods_sourceProjectId": Common.get_random('sourceProjectId'),
			}
		)
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get('jfqjy_3_periods_sourceUserId'),
				"transactionId": self.r.get('jfqjy_3_periods_transactionId')
			}
		)
		...
		param['personalInfo'].update(
			{
				"cardNum": self.r.get('jfqjy_3_periods_cardNum'),
				"custName": self.r.get('jfqjy_3_periods_custName'),
				"phone": self.r.get('jfqjy_3_periods_phone')
			}
		)
		...
		param.update(
			{
				...
				"transactionId": self.r.get('jfqjy_3_periods_transactionId'),
				"associationId": self.r.get('jfqjy_3_periods_projectId'),
	...
	def test_102_query_apply_result(self):
		...
		GetSqlData.change_project_audit_status(
			project_id=self.r.get('jfqjy_3_periods_projectId'),
			environment=self.env
		)
	...
	def test_105_image_upload(self):
		...
		param = json.loads(data[0]['param'])
		param.update({"associationId": self.r.get('jfqjy_3_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get("jfqjy_3_periods_sourceUserId"),
				"contractId": self.r.get("jfqjy_3_periods_contractId")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_107_calculate(self):
		...
		param.update(
			{
				"sourceUserId": self.r.get("jfqjy_3_periods_sourceUserId"),
				"transactionId": self.r.get("jfqjy_3_periods_sourceProjectId"),
				"sourceProjectId": self.r.get("jfqjy_3_periods_sourceProjectId"),
				"projectId": self.r.get("jfqjy_3_periods_projectId")
			}
		)
	...
	def test_108_loan_pfa(self):
		...
		param = json.loads(data[0]['param'])
		self.r.set("jfqjy_3_periods_loan_serviceSn", Common.get_random("serviceSn"))
		param.update(
			{
				"sourceProjectId": self.r.get("jfqjy_3_periods_sourceProjectId"),
				"projectId": self.r.get("jfqjy_3_periods_projectId"),
				"sourceUserId": self.r.get("jfqjy_3_periods_sourceUserId"),
				"serviceSn": self.r.get("jfqjy_3_periods_loan_serviceSn"),
				"id": self.r.get('jfqjy_3_periods_cardNum'),
				"accountName": self.r.get("jfqjy_3_periods_custName"),
				"amount": 45000
			}
		)
		if len(data[0]['headers']) == 0:
	...
	def test_109_loan_query(self):
		...
		GetSqlData.loan_set(environment=self.env, project_id=self.r.get('jfqjy_3_periods_projectId'))
		param = json.loads(data[0]['param'])
	...
	def test_110_query_repayment_plan(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(False, self.env, self.r.get("jfqjy_3_periods_projectId"))
	...
	def test_113_offline_repay_repayment(self):
		...
		repayment_plan_list = self.r.get("jfqjy_3_periods_repayment_plan")
		success_amount = 0.00
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(True, self.env, self.r.get("jfqjy_3_periods_projectId"), param)
	...
	def test_114_offline_nrepay_early_settlement(self):
		...
		repayment_plan_list = json.loads(self.r.get("jfqjy_3_periods_early_settlement_repayment_plan"))
		success_amount = 0.00
		repayment_detail_list = []
	...
	def test_116_return(self):
		...
		repayment_plan_list = json.loads(self.r.get("jfqjy_3_periods_return_repayment_plan"))
		success_amount = 0.00
		repayment_detail_list = []
		for i in repayment_plan_list:
			plan_detail = {
