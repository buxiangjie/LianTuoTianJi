...
logger = Logger(logger="test_jfqyl_12_periods_tp").getlog()


class Jfqyl12Tp(unittest.TestCase):
	...
	def test_100_apply(self):
		...
		Common.p2p_get_userinfo('jfqyl_12_periods', self.env)
		self.r.mset(
			{
				"jfqyl_12_periods_sourceUserId": Common.get_random('userid'),
				"jfqyl_12_periods_transactionId": Common.get_random('transactionId'),
				"jfqyl_12_periods_phone": Common.get_random('phone'),
				"jfqyl_12_periods_sourceProjectId": Common.get_random('sourceProjectId'),
			}
		)
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get('jfqyl_12_periods_sourceUserId'),
				"transactionId": self.r.get('jfqyl_12_periods_transactionId')
			}
		)
		...
		param['personalInfo'].update(
			{
				"cardNum": self.r.get('jfqyl_12_periods_cardNum'),
				"custName": self.r.get('jfqyl_12_periods_custName'),
				"phone": self.r.get('jfqyl_12_periods_phone')
			}
		)
		...
		param.update(
			{
				...
				"transactionId": self.r.get('jfqyl_12_periods_transactionId'),
				"associationId": self.r.get('jfqyl_12_periods_projectId'),
	...
	def test_102_query_apply_result(self):
		...
		GetSqlData.change_project_audit_status(
			project_id=self.r.get('jfqyl_12_periods_projectId'),
			environment=self.env
		)
	...
	def test_103_sign_borrow(self):
		...
		self.r.set("jfqyl_12_periods_contractId", rep['content']['contractId'])
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
	...
	def test_105_image_upload(self):
		...
		param = json.loads(data[0]['param'])
		param.update({"associationId": self.r.get('jfqyl_12_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get("jfqyl_12_periods_sourceUserId"),
				"contractId": self.r.get("jfqyl_12_periods_contractId")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_108_loan_pfa(self):
		...
		param = json.loads(data[0]['param'])
		self.r.set("jfqyl_12_periods_loan_serviceSn", Common.get_random("serviceSn"))
		param.update(
			{
				"sourceProjectId": self.r.get("jfqyl_12_periods_sourceProjectId"),
				"projectId": self.r.get("jfqyl_12_periods_projectId"),
	...
	def test_109_loan_query(self):
		"""放款结果查询"""
		GetSqlData.loan_set(environment=self.env, project_id=self.r.get('jfqyl_12_periods_projectId'))
		data = excel_table_byname(self.file, 'pfa_query')
		param = json.loads(data[0]['param'])
		param.update({"serviceSn": self.r.get("jfqyl_12_periods_loan_serviceSn")})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_110_query_repayment_plan(self):
		...
		self.r.set("jfqyl_12_periods_repayment_plan", json.dumps(rep['content']['repaymentPlanList']))
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
	...
	def test_111_calculate(self):
		...
		self.r.set(
			"jfqyl_12_periods_early_settlement_repayment_plan",
	...
	def test_112_calculate(self):
		...
		self.r.set(
			"jfqyl_12_periods_return_repayment_plan",
			json.dumps(rep['content']['repaymentPlanList'])
		)
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
	...
	def test_113_offline_repay_repayment(self):
		...
		param = json.loads(data[0]['param'])
		period = 1
		plan_pay_date = GetSqlData.get_repayment_plan_date(project_id=self.r.get("jfqyl_12_periods_projectId"),
														   environment=self.env, repayment_plan_type=1, period=period)
		repayment_plan_list = self.r.get("jfqyl_12_periods_repayment_plan")
		success_amount = 0.00
	...
	def test_114_offline_nrepay_early_settlement(self):
		...
		repayment_plan_list = json.loads(self.r.get("jfqyl_12_periods_early_settlement_repayment_plan"))
		success_amount = 0.00
		repayment_detail_list = []
	...
	def test_116_return(self):
		...
		plan_pay_date = GetSqlData.get_repayment_plan_date(project_id=self.r.get("jfqyl_12_periods_projectId"),
														   environment=self.env, repayment_plan_type=1, period=1)
		repayment_plan_list = json.loads(self.r.get("jfqyl_12_periods_return_repayment_plan"))
		success_amount = 0.00
		repayment_detail_list = []
		for i in repayment_plan_list:
			plan_detail = {
