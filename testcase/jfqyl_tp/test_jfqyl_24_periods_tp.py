...
class Jfqyl24Tp(unittest.TestCase):
	...
	def test_109_loan_query(self):
		"""放款结果查询"""
		GetSqlData.loan_set(environment=self.env, project_id=self.r.get('jfqyl_24_periods_projectId'))
	...
	def test_113_offline_repay_repayment(self):
		...
		param = json.loads(data[0]['param'])
		period = 1
		plan_pay_date = GetSqlData.get_repayment_plan_date(project_id=self.r.get("jfqyl_24_periods_projectId"),
