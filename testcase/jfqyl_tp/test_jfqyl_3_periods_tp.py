...
class Jfqyl3Tp(unittest.TestCase):
	...
	def test_103_sign_borrow(self):
		...
		self.r.set("jfqyl_3_periods_contractId", rep['content']['contractId'])
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
	...
	def test_110_query_repayment_plan(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(False, self.env, self.r.get("jfqyl_3_periods_projectId"))
		self.r.set("jfqyl_3_periods_repayment_plan", json.dumps(rep['content']['repaymentPlanList']))
	...
	def test_113_offline_repay_repayment(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(True, self.env, self.r.get("jfqyl_3_periods_projectId"), param)
