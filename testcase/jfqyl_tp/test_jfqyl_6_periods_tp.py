...
class Jfqyl6Tp(unittest.TestCase):
	...
	def test_110_query_repayment_plan(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(False, self.env, self.r.get("jfqyl_6_periods_projectId"))
	...
	def test_113_offline_repay_repayment(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(True, self.env, self.r.get("jfqyl_6_periods_projectId"), param)
	...
	def test_116_return(self):
		...
		data = excel_table_byname(self.file, 'offline_repay')
		param = json.loads(data[0]['param'])
		plan_pay_date = GetSqlData.get_repayment_plan_date(project_id=self.r.get("jfqyl_6_periods_projectId"),
