...
logger = Logger(logger="test_jfqyl_9_periods_tp").getlog()


class Jfqyl9Tp(unittest.TestCase):
	...
	def test_100_apply(self):
		...
		param.update(
			{
				"sourceProjectId": self.r.get('jfqyl_9_periods_sourceProjectId'),
		...
		param['personalInfo'].update(
			{
				...
				"custName": self.r.get('jfqyl_9_periods_custName'),
				"phone": self.r.get('jfqyl_9_periods_phone')
			}
		)
		...
		self.r.set('jfqyl_9_periods_projectId', projectId)
	...
	def test_102_query_apply_result(self):
		...
		param.update(
			{
				"sourceProjectId": self.r.get('jfqyl_9_periods_sourceProjectId'),
				"projectId": self.r.get('jfqyl_9_periods_projectId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_103_sign_borrow(self):
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("jfqyl_9_periods_contractId", json.loads(rep.text)['content']['contractId'])
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))
	...
	def test_105_image_upload(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		param.update({"associationId": self.r.get('jfqyl_9_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get("jfqyl_9_periods_sourceUserId"),
				"contractId": self.r.get("jfqyl_9_periods_contractId")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_107_calculate(self):
		...
		param.update(
			{
				"sourceUserId": self.r.get("jfqyl_9_periods_sourceUserId"),
				"transactionId": self.r.get("jfqyl_9_periods_sourceProjectId"),
				"sourceProjectId": self.r.get("jfqyl_9_periods_sourceProjectId"),
				"projectId": self.r.get("jfqyl_9_periods_projectId")
			}
		)
	...
	def test_108_loan_pfa(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		self.r.set("jfqyl_9_periods_loan_serviceSn", Common.get_random("serviceSn"))
		param.update(
			{
				"sourceProjectId": self.r.get("jfqyl_9_periods_sourceProjectId"),
				"projectId": self.r.get("jfqyl_9_periods_projectId"),
				"sourceUserId": self.r.get("jfqyl_9_periods_sourceUserId"),
				"serviceSn": self.r.get("jfqyl_9_periods_loan_serviceSn"),
				"id": self.r.get('jfqyl_9_periods_cardNum'),
				"accountName": self.r.get("jfqyl_9_periods_custName"),
				"amount": 33333.33
			}
		)
		if len(data[0]['headers']) == 0:
	...
	def test_109_loan_query(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		param.update({"serviceSn": self.r.get("jfqyl_9_periods_loan_serviceSn")})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_110_query_repayment_plan(self):
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("jfqyl_9_periods_repayment_plan", json.dumps(json.loads(rep.text)['content']['repaymentPlanList']))
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))
	...
	def test_111_calculate(self):
		...
		logger.info("返回信息:%s" % rep.text)
		self.r.set(
			"jfqyl_9_periods_early_settlement_repayment_plan",
	...
	def test_112_calculate(self):
		...
		logger.info("返回信息:%s" % rep.text)
		self.r.set(
			"jfqyl_9_periods_return_repayment_plan",
			json.dumps(json.loads(rep.text)['content']['repaymentPlanList'])
		)
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))
