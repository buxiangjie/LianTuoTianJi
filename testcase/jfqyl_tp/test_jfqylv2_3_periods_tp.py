...
class Jfqylv23Tp(unittest.TestCase):
	...
	def test_103_sign_borrow(self):
		...
		self.r.set("jfqylv2_3_periods_contractId", rep['content']['contractId'])
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
	...
	def test_109_loan_query(self):
		"""放款结果查询"""
		GetSqlData.loan_set(environment=self.env, project_id=self.r.get('jfqylv2_3_periods_projectId'))
	...
	def test_110_query_repayment_plan(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(False, self.env, self.r.get("jfqylv2_3_periods_projectId"))
		self.r.set("jfqylv2_3_periods_repayment_plan", json.dumps(rep['content']['repaymentPlanList']))
