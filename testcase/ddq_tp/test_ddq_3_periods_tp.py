...
class Ddq3Tp(unittest.TestCase):
	...
	def test_102_query_apply_result(self):
		"""进件结果查询"""
		Assert.check_column("wxjk_project", self.env, self.r.get('ddq_3_periods_projectId'))
	...
	def test_110_query_repayment_plan(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(False, self.env, self.r.get("ddq_3_periods_projectId"))
	...
	def test_112_offline_repay_repayment(self):
		...
		self.assertEqual(rep['resultCode'], int(data[0]['resultCode']))
		Assert.check_repayment(True, self.env, self.r.get("ddq_3_periods_projectId"), param)
