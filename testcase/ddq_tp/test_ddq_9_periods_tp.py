...
class Ddq9Tp(unittest.TestCase):
	...
	def test_102_query_apply_result(self):
		"""进件结果查询"""
		Assert.check_column("wxjk_project", self.env, self.r.get('ddq_9_periods_projectId'))
	...
	def test_113_offline_repay_repayment(self):
		...
		param = json.loads(data[0]['param'])
		period = 1
		plan_pay_date = GetSqlData.get_repayment_plan_date(project_id=self.r.get("ddq_9_periods_projectId"),
