...
class Jfqyl18Tp(unittest.TestCase):
	...
	def test_100_apply(self):
		...
		Common.p2p_get_userinfo('jfqyl_18_periods', self.env)
		self.r.mset(
			{
		...
		param.update(
			{
				"sourceProjectId": self.r.get('jfqyl_18_periods_sourceProjectId'),
				"sourceUserId": self.r.get('jfqyl_18_periods_sourceUserId'),
				"transactionId": self.r.get('jfqyl_18_periods_transactionId')
			}
		)
		...
		param['personalInfo'].update(
			{
				"cardNum": self.r.get('jfqyl_18_periods_cardNum'),
				"custName": self.r.get('jfqyl_18_periods_custName'),
				"phone": self.r.get('jfqyl_18_periods_phone')
			}
		)
		...
		self.r.set('jfqyl_18_periods_projectId', projectId)
		...
		param.update(
			{
				...
				"transactionId": self.r.get('jfqyl_18_periods_transactionId'),
				"associationId": self.r.get('jfqyl_18_periods_projectId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_102_query_apply_result(self):
		...
		GetSqlData.change_project_audit_status(
			project_id=self.r.get('jfqyl_18_periods_projectId'),
			enviroment=self.env
		)
		...
		param.update(
			{
				"sourceProjectId": self.r.get('jfqyl_18_periods_sourceProjectId'),
				"projectId": self.r.get('jfqyl_18_periods_projectId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_103_sign_borrow(self):
		...
		self.r.set("jfqyl_18_periods_contractId", rep['content']['contractId'])
		assert rep['resultCode'] == int(data[0]['resultCode'])
	...
	def test_105_image_upload(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		param.update({"associationId": self.r.get('jfqyl_18_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get("jfqyl_18_periods_sourceUserId"),
				"contractId": self.r.get("jfqyl_18_periods_contractId")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_107_calculate(self):
		...
		param.update(
			{
				"sourceUserId": self.r.get("jfqyl_18_periods_sourceUserId"),
				"transactionId": self.r.get("jfqyl_18_periods_sourceProjectId"),
				"sourceProjectId": self.r.get("jfqyl_18_periods_sourceProjectId"),
				"projectId": self.r.get("jfqyl_18_periods_projectId")
			}
		)
	...
	def test_108_loan_pfa(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		self.r.set("jfqyl_18_periods_loan_serviceSn", Common.get_random("serviceSn"))
		param.update(
			{
				"sourceProjectId": self.r.get("jfqyl_18_periods_sourceProjectId"),
				"projectId": self.r.get("jfqyl_18_periods_projectId"),
				"sourceUserId": self.r.get("jfqyl_18_periods_sourceUserId"),
				"serviceSn": self.r.get("jfqyl_18_periods_loan_serviceSn"),
				"id": self.r.get('jfqyl_18_periods_cardNum'),
				"accountName": self.r.get("jfqyl_18_periods_custName"),
				"amount": 33333.33
			}
		)
		if len(data[0]['headers']) == 0:
	...
	def test_109_loan_query(self):
		"""放款结果查询"""
		GetSqlData.loan_set(enviroment=self.env, project_id=self.r.get('jfqyl_18_periods_projectId'))
		data = excel_table_byname(self.excel, 'pfa_query')
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		param.update({"serviceSn": self.r.get("jfqyl_18_periods_loan_serviceSn")})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_110_query_repayment_plan(self):
		...
		self.r.set("jfqyl_18_periods_repayment_plan", json.dumps(rep['content']['repaymentPlanList']))
		assert rep['resultCode'] == int(data[0]['resultCode'])
	...
	def test_113_offline_repay_repayment(self):
		...
		repayment_plan_list = self.r.get("jfqyl_18_periods_repayment_plan")
		success_amount = 0.00
	...
	def test_114_offline_nrepay_early_settlement(self):
		...
		repayment_plan_list = json.loads(self.r.get("jfqyl_18_periods_early_settlement_repayment_plan"))
		success_amount = 0.00
		repayment_detail_list = []
	...
	@unittest.skip("-")
	# @unittest.skipUnless(sys.argv[4] == "return", "-")
	def test_116_return(self):
		...
		repayment_plan_list = json.loads(self.r.get("jfqyl_18_periods_return_repayment_plan"))
		success_amount = 0.00
		repayment_detail_list = []
		for i in repayment_plan_list:
			plan_detail = {
	...
	def test_117_capital_flow(self):
		...
		assert response_data['resultCode'] == int(data[0]['resultCode'])
