...
class TestJfx18PeriodTp:
	...
	def test_100_credit_apply(self, r, env):
		...
		r.mset(
			{
				"jfx_18_periods_sourceUserId": Common.get_random('userid'),
				'jfx_18_periods_transactionId': Common.get_random('transactionId'),
				"jfx_18_periods_phone": Common.get_random('phone'),
				"jfx_18_periods_firstCreditDate": Common.get_time()
			}
		)
		param = json.loads(data[0]['param'])
		param['personalInfo'].update(
			{
				"cardNum": r.get('jfx_18_periods_cardNum'),
				"custName": r.get('jfx_18_periods_custName'),
				"phone": r.get('jfx_18_periods_phone'),
				"isDoctor": 0,
		...
		param.update(
			{
				"sourceUserId": r.get('jfx_18_periods_sourceUserId'),
				"serviceSn": Common.get_random('serviceSn'),
				"transactionId": r.get('jfx_18_periods_transactionId')
			}
		)
		...
		r.mset(
			{
				"jfx_18_periods_creditId": rep['content']['creditId'],
				"jfx_18_periods_userId": rep['content']['userId']
			}
		)
	...
	def test_101_query_result(self, r, env):
		"""授信结果查询"""
		Asset.check_column("jfx_credit", self.env, self.r.get("jfx_18_periods_creditId"))
		GetSqlData.credit_set(
			environment=env,
			credit_id=r.get("jfx_18_periods_creditId")
		)
		data = excel_table_byname(self.file, 'credit_query_result')
		param = json.loads(data[0]['param'])
		param.update({"creditId": r.get('jfx_18_periods_creditId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_102_query_user_amount(self, r, env):
		...
		param.update(
			{
				"sourceUserId": r.get('jfx_18_periods_sourceUserId'),
				"userId": r.get('jfx_18_periods_userId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
		...
		param.update(
			{
				...
				"transactionId": r.get('jfx_18_periods_transactionId'),
				"associationId": r.get('jfx_18_periods_creditId'),
	...
	def test_104_project_apply(self, r, env):
		...
		param = json.loads(data[0]['param'])
		r.set('jfx_18_periods_sourceProjectId', Common.get_random('sourceProjectId'))
		param.update(
			{
				"sourceProjectId": r.get('jfx_18_periods_sourceProjectId'),
				"sourceUserId": r.get('jfx_18_periods_sourceUserId'),
				"transactionId": r.get('jfx_18_periods_transactionId')
			}
		)
		...
		r.set("jfx_18_periods_corporateAccountName", param['cardInfo']['corporateAccountName'])
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_105_query_apply_result(self, r, env):
		"""进件结果查询"""
		Asset.check_column("jfx_project", self.env, self.r.get("jfx_18_periods_projectId"))
		GetSqlData.change_project_audit_status(
			project_id=r.get('jfx_18_periods_projectId'),
			environment=env
		)
		...
		param.update(
			{
				"sourceProjectId": r.get('jfx_18_periods_sourceProjectId'),
				"projectId": r.get('jfx_18_periods_projectId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
		...
		param.update(
			{
				...
				"sourceContractId": Common.get_random('userid'),
				"transactionId": r.get('jfx_18_periods_transactionId'),
				"associationId": r.get('jfx_18_periods_projectId'),
				"content": Common.get_json_data('data', 'credit_sign.json').get("content")
			}
		)
		if len(data[0]['headers']) == 0:
		...
		param.update(
			{
				...
				"sourceUserId": r.get('jfx_18_periods_sourceUserId'),
				"serviceSn": Common.get_random('serviceSn'),
				"accountName": r.get("jfx_18_periods_corporateAccountName"),
				"bankCode": 86,
				"amount": 84920.00,
				"accountNo": "6214830173648711"  # 6227002432220410613
			}
		)
		r.set("jfx_18_periods_pfa_serviceSn", param['serviceSn'])
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_109_pfa_query(self, r, env):
		...
		param = json.loads(data[0]['param'])
		param.update({"serviceSn": r.get('jfx_18_periods_pfa_serviceSn')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_110_early_settlement(self, r, env):
		...
		param.update(
			{
				"sourceUserId": r.get("jfx_18_periods_sourceUserId"),
				"transactionId": r.get("jfx_18_periods_sourceProjectId"),
				"sourceProjectId": r.get("jfx_18_periods_sourceProjectId"),
				"projectId": r.get("jfx_18_periods_projectId"),
				"businessType": 2
			}
		...
		r.set(
			"jfx_18_periods_early_settlement_repayment_plan",
	...
	def test_111_query_repaymentplan(self, r, env):
		...
		param = json.loads(data[0]['param'])
		param.update({"projectId": r.get('jfx_18_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		assert int(data[0]['resultCode']) == rep['resultCode']
		r.set("jfx_18_periods_repayment_plan", json.dumps(rep['content']['repaymentPlanList']))
	...
	def test_112_repayment(self, r, env):
		...
		param = json.loads(data[0]['param'])
		repayment_plan_list = r.get("jfx_18_periods_repayment_plan")
		success_amount = 0.00
	...
	def test_114_offline_settle(self, r, env):
		...
		repayment_plan_list = r.get("jfx_18_periods_early_settlement_repayment_plan")
		success_amount = 0.00
		repayment_detail_list = []
		for i in json.loads(repayment_plan_list):
			plan_detail = {
	...
	def test_115_capital_flow(self, r, env):
		...
		success_amount = GetSqlData.get_repayment_amount(
			project_id=r.get("jfx_18_periods_projectId"), environment=env, period=1)
		param.update(
			{
				"serviceSn": Common.get_random("serviceSn"),
				"projectId": r.get("jfx_18_periods_projectId"),
