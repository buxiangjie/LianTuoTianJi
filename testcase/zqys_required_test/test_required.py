...
class CreditNone(unittest.TestCase):
	file = Config().get_item('File', 'zqys_required_case_file')
...
class ApplyNone(unittest.TestCase):
	...
	def credit(self, red):
		print("授信------")
		data = excel_table_byname(Config().get_item('File', 'zqys_case_file'), 'credit')
	...
	def query_result(self, red):
		...
		data = excel_table_byname(Config().get_item('File', 'zqys_case_file'), 'query_result')
		param = json.loads(data[0]['param'])
