...
logger = Logger(logger="test_rmkj_3_periods_tp").getlog()


class Rmkj3Tp(unittest.TestCase):
	...
	def test_100_apply(self):
		...
		Common.p2p_get_userinfo('rmkj_3_periods', self.env)
		self.r.mset(
			{
		...
		param.update(
			{
				"sourceProjectId": self.r.get('rmkj_3_periods_sourceProjectId'),
				"sourceUserId": self.r.get('rmkj_3_periods_sourceUserId'),
				"transactionId": self.r.get('rmkj_3_periods_transactionId')
			}
		)
		param['personalInfo'].update(
			{
				"cardNum": self.r.get('rmkj_3_periods_cardNum'),
				"custName": self.r.get('rmkj_3_periods_custName'),
				"phone": self.r.get('rmkj_3_periods_phone')
			}
		)
		...
		param['bindingCardInfo'].update(
			{
				"bankCardNo": self.r.get('rmkj_3_periods_bankcard'),
				"bankPhone": self.r.get('rmkj_3_periods_phone'),
			}
		)
		...
		self.r.set('rmkj_3_periods_projectId', projectId)
		...
		param.update(
			{
				...
				"transactionId": self.r.get('rmkj_3_periods_transactionId'),
				"associationId": self.r.get('rmkj_3_periods_projectId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_102_query_apply_result(self):
		...
		GetSqlData.change_project_audit_status(
			project_id=self.r.get('rmkj_3_periods_projectId'),
			enviroment=self.env
		)
		...
		param.update(
			{
				"sourceProjectId": self.r.get('rmkj_3_periods_sourceProjectId'),
				"projectId": self.r.get('rmkj_3_periods_projectId')
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_103_sign_borrow(self):
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("rmkj_3_periods_contractId", json.loads(rep.text)['content']['contractId'])
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))

	def test_1033_sign_repayment(self):
		"""上传还款计划文件"""
		data = excel_table_byname(self.excel, 'contract_sign')
		print("接口名称:%s" % data[0]['casename'])
		param = Common.get_json_data('data', 'rmkj_sign_borrow.json')
	...
	def test_104_image_upload(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		param.update({"associationId": self.r.get('rmkj_3_periods_projectId')})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
		...
		param.update(
			{
				...
				"sourceUserId": self.r.get("rmkj_3_periods_sourceUserId"),
				"contractId": self.r.get("rmkj_3_periods_contractId")
			}
		)
		if len(data[0]['headers']) == 0:
			headers = None
	...
	def test_106_sign(self):
		...
		param.update(
			{
				...
				"requestTime": Common.get_time("-"),
				"sourceUserId": self.r.get("rmkj_3_periods_sourceUserId"),
				"name": self.r.get("rmkj_3_periods_custName"),
				"cardNo": self.r.get("rmkj_3_periods_cardNum"),
				# "name": "卜祥杰",
				# "cardNo": "372301199509074811",
				# "bankCode": "BOC",
				"bankNo": self.r.get("rmkj_3_periods_bankcard"),
				# "bankName": "中国银行",
				"mobile": self.r.get("rmkj_3_periods_phone"),
				# "mobile": "18366582857",
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("rmkj_3_periods_signTaskId", json.loads(rep.text)['data']['signTaskId'])
		...
		param.update(
			{
				...
				"requestTime": Common.get_time("-"),
				"sourceUserId": self.r.get("rmkj_3_periods_sourceUserId"),
				"signTaskId": self.r.get("rmkj_3_periods_signTaskId"),
				"smsCode": "849201"
			}
		)
		if len(data[0]['headers']) == 0:
	...
	def test_1091_calculate(self):
		...
		param.update(
			{
				"sourceUserId": self.r.get("rmkj_3_periods_sourceUserId"),
				"transactionId": self.r.get("rmkj_3_periods_sourceProjectId"),
				"sourceProjectId": self.r.get("rmkj_3_periods_sourceProjectId"),
				"projectId": self.r.get("rmkj_3_periods_projectId")
			}
		)
	...
	def test_110_loan_pfa(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		self.r.set("rmkj_3_periods_loan_serviceSn", Common.get_random("serviceSn"))
		param.update(
			{
				"sourceProjectId": self.r.get("rmkj_3_periods_sourceProjectId"),
				"projectId": self.r.get("rmkj_3_periods_projectId"),
				"sourceUserId": self.r.get("rmkj_3_periods_sourceUserId"),
				"serviceSn": self.r.get("rmkj_3_periods_loan_serviceSn"),
	...
	def test_111_loan_query(self):
		"""放款结果查询"""
		GetSqlData.loan_set(enviroment=self.env, project_id=self.r.get('rmkj_3_periods_projectId'))
		data = excel_table_byname(self.excel, 'pfa_query')
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		param.update({"serviceSn": self.r.get("rmkj_3_periods_loan_serviceSn")})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_112_query_repayment_plan(self):
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("rmkj_3_periods_repayment_plan", json.dumps(json.loads(rep.text)['content']['repaymentPlanList']))
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))
	...
	# @unittest.skip("跳过")
	def test_115_deduction_apply(self):
		"""主动还款:正常还一期"""
		data = excel_table_byname(self.excel, 'deduction_apply')
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		repayment_plan_list = self.r.get("rmkj_3_periods_repayment_plan")
		success_amount = 0.00
		...
		print("返回json:%s" % rep.text)
		logger.info("返回信息:%s" % rep.text)
		self.r.set("rmkj_3_periods_deductionTaskId", json.loads(rep.text)['content']['deductionTaskId'])
		self.assertEqual(json.loads(rep.text)['resultCode'], int(data[0]['resultCode']))

	# @unittest.skip("跳过")
	@unittest.skipUnless(sys.argv[4] == "all_periods", "-")
	def test_116_deduction_apply_all_periods(self):
		"""主动还款:连续还款整笔结清"""
		data = excel_table_byname(self.excel, 'deduction_apply')
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
	...
	# @unittest.skip("跳过")
	@unittest.skipUnless(sys.argv[4] == "early_settlement", "-")
	def test_117_deduction_apply(self):
		"""主动还款:提前全部结清"""
		data = excel_table_byname(self.excel, 'deduction_apply')
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		repayment_plan_list = self.r.get("rmkj_3_periods_early_settlement_repayment_plan")
		success_amount = 0.00
		repayment_detail_list = []
		for i in json.loads(repayment_plan_list):
			plan_detail = {
	...
	def test_119_deduction_query(self):
		...
		print("接口名称:%s" % data[0]['casename'])
		param = json.loads(data[0]['param'])
		param.update({"deductionTaskId": self.r.get("rmkj_3_periods_deductionTaskId")})
		if len(data[0]['headers']) == 0:
			headers = None
		else:
			headers = json.loads(data[0]['headers'])
	...
	def test_122_refunds(self):
		...
		repayment_plan_list = self.r.get("rmkj_3_periods_refunds_repayment_plan")
		success_amount = 0.00
		repayment_detail_list = []
		for i in json.loads(repayment_plan_list):
			plan_detail = {
